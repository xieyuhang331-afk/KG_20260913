...
describe("Organization Frontend Prototype Alignment V1", () => {
	...
	it("consumes the four-level governance tree without tenant or administrator PII", async () => {
		...
		expect(result[0].children[0].children[0].children[0].org_type).toBe(
			"county",
		);
	...
	it.each([
	...
	])("allows platform read access for %s", async (role) => {
		setCurrentUser({
			id: 2,
			role,
			tenant_id: null,
		...
		expect(
			await screen.findByRole("heading", { name: "组织架构管理" }),
		).toBeInTheDocument();
	});
	...
	it("renders the aligned platform shell without unavailable prototype metrics", async () => {
		...
		expect(
			screen.getByRole("navigation", { name: "平台治理主导航" }),
		).toBeInTheDocument();
	...
	it("keeps the organization hierarchy operable from the keyboard", async () => {
		...
		expect(
			await screen.findByRole("button", { name: "浙江省" }),
	...
	it("presents district and county nodes at the same county contract level", async () => {
		...
		expect(
			await screen.findByRole("heading", { name: "杭州市" }),
		).toBeInTheDocument();
		expect(
			await screen.findByText("总部 / 省 / 市 / 区/县"),
		).toBeInTheDocument();
		...
		expect(
			screen.getByRole("button", { name: "新增区/县" }),
		).toBeInTheDocument();
	});
	...
	it("creates a city child as county and never offers another platform level below county", async () => {
		...
		renderPlatform();
		expect(
			await screen.findByRole("heading", { name: "滨江区" }),
		).toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: "新增区/县" }),
		).not.toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: "创建下级组织" }),
		).not.toBeInTheDocument();
	});

	it.each([USER_ROLES.provinceAdmin, USER_ROLES.cityAdmin])(
		...
		async (role) => {
			...
			expect(
				screen.queryByRole("button", { name: "修改组织" }),
			).not.toBeInTheDocument();
		},
	);
	...
	it.each([
		...
		"preserves PATCH administrator three-state semantics: %s",
	...
	it("consumes paged administrator candidates without retaining extra PII fields", async () => {
		...
		expect(fetchMock.mock.calls[0][0]).toBe(
			"/api/v1/platform/organizations/21/admin-candidates?page_size=100",
		);
	...
	it("ignores a late administrator candidate response after the panel lifecycle changes", async () => {
		...
		const fetchMock = vi.fn((url: string) => {
			if (url.includes("/tree"))
				return Promise.resolve(apiSuccess({ items: treeData() }));
			if (url.includes("/tenants"))
				return Promise.resolve(apiSuccess(tenantPage(null)));
	...
	it.each([
		...
		"clears administrator candidates when switching directly to %s",
		async (actionName, panelActionName) => {
			...
			expect(
				screen.getByRole("button", { name: panelActionName }),
			).toBeInTheDocument();
	...
	it("does not automatically replay a 409 mutation and refreshes read state", async () => {
		...
		const fetchMock = vi.fn((url: string) => {
			if (url.endsWith("/deactivate")) {
				mutationCalls += 1;
				return Promise.resolve(
					apiResponse(409, { detail: "internal-write-detail" }),
				);
			}
			if (url.includes("/tree"))
				return Promise.resolve(apiSuccess({ items: treeData() }));
			if (url.includes("/tenants"))
				return Promise.resolve(apiSuccess(tenantPage("OPAQUE_NEXT")));
			return Promise.resolve(
				apiSuccess(
					detailData({
						id: 21,
		...
		expect(
			await screen.findByText(
				"组织版本或状态已变化，已刷新最新数据，请确认后重试。",
			),
		).toBeInTheDocument();
		expect(mutationCalls).toBe(1);
		await waitFor(() =>
			expect(
				fetchMock.mock.calls.filter(([url]) => String(url).includes("/tree"))
					.length,
			).toBeGreaterThan(1),
		);
	});
	...
	it("loads archived siblings before enabling atomic child ordering", async () => {
		...
		const visibleTree = [
			node(11, "华东总部", "headquarter", "active", "canonical", [
				visibleChild,
			]),
		];
		const completeTree = [
		...
		const fetchMock = vi.fn((url: string, options?: RequestInit) => {
			if (url.includes("/tree?include_archived=true"))
				return Promise.resolve(apiSuccess({ items: completeTree }));
			if (url.includes("/tree"))
				return Promise.resolve(apiSuccess({ items: visibleTree }));
			if (url.includes("/tenants"))
				return Promise.resolve(apiSuccess(tenantPage(null)));
		...
		await userEvent.click(
			screen.getByRole("checkbox", { name: "包含已归档历史节点" }),
		);
		await waitFor(() =>
			expect(
				screen.getByRole("button", { name: "调整同级排序" }),
			).toBeEnabled(),
		);
		...
		const orderCall = fetchMock.mock.calls.find(
			([url, options]) =>
				String(url).endsWith("/children/order") &&
	...
	it("keeps legacy and archived nodes mutation-free", async () => {
		...
		renderPlatform();
		expect(
			await screen.findByText("只读节点不允许治理操作"),
		).toBeInTheDocument();
		expect(
			screen.queryByRole("button", { name: "停用组织" }),
		).not.toBeInTheDocument();
	});
	...
	it("renders the aligned institution shell and read-only governance path", async () => {
		...
		expect(
			await screen.findByRole("heading", { name: "机构组织资料" }),
		).toBeInTheDocument();
		expect(
	...
	it("renders the unassigned institution state for null organization and an empty path", async () => {
		...
		renderInstitution();
		expect(
			(await screen.findAllByText("暂未分配平台组织")).length,
	...
	it("keeps P1 and Identity Review routes renderable", async () => {
		...
		const platformRender = renderWithClient(
		...
		<MemoryRouter initialEntries={["/institution/organization"]}>
			<Routes>
				<Route
					...
					element={<InstitutionOrganizationPage />}
				/>
...
function mockPlatformReadApis(tree = treeData(), detail = detailData()) {
	const fetchMock = vi.fn((url: string, _options?: RequestInit) => {
		if (url.includes("/tree"))
			return Promise.resolve(apiSuccess({ items: tree }));
		if (url.includes("/tenants"))
			return Promise.resolve(apiSuccess(tenantPage("OPAQUE_NEXT")));
...
function treeData(
	...
	const county = node(
		...
		"滨江区",
		"county",
		options.includeStates ? "archived" : "active",
		"canonical",
		[],
	);
	...
	const city = node(
		...
		"杭州市",
		"city",
		options.includeStates ? "inactive" : "active",
		"canonical",
		[county, countyPeer],
	);
	const province = node(
		...
		"province",
		"active",
		options.includeStates ? "legacy" : "canonical",
		[city],
	);
	return [
		node(
			...
			"华东总部",
			"headquarter",
			options.rootStatus ?? "active",
			options.rootMode ?? "canonical",
			[province],
		),
	];
}
...
function mockApi(status: number, data: unknown) {
	const fetchMock = vi
		.fn()
		.mockResolvedValue(
			status >= 400 ? apiResponse(status, data) : apiSuccess(data),
		);
	vi.stubGlobal("fetch", fetchMock);
	return fetchMock;
}
