...
export function MemberEnrollmentDetailPage() {
	...
	const [caseDetail, setCaseDetail] = useState<PreparingServiceCase | null>(
		null,
	);
	...
	const [identityDecision, setIdentityDecision] = useState<
		"CHECKED" | "NEEDS_CORRECTION" | "REJECTED"
	>("CHECKED");
								...
								{detail.identity ? (
									<>
										<dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
											<Field
												label="脱敏证件"
												value={detail.identity.id_masked}
											/>
											<Field
												label="当前状态"
												value={identityStatusLabel(detail.identity.status)}
											/>
											<Field
												label="实名材料版本"
										...
										>
											<label className="block text-sm font-medium">
												...
												<select
													...
													onChange={(event) =>
														setIdentityDecision(
															event.target.value as typeof identityDecision,
														)
													}
											...
											{identityDecision === "NEEDS_CORRECTION" ? (
												<>
													...
													<fieldset className="flex gap-4 text-sm">
														...
														{(["real_name", "id_number"] as const).map(
															(field) => (
																<label key={field}>
																	<input
										...
										<label className="block text-sm font-medium">
											...
											>
												...
												{therapists.map((item) => (
													...
													>
														{item.display_name ?? compactId(item.therapist_id)}{" "}
								...
								{caseDetail ? (
									<>
										<dl className="mt-4 space-y-3 text-sm">
											...
											<Field
												label="创建时间"
												value={formatTime(caseDetail.created_at)}
											/>
										</dl>
										<Link
