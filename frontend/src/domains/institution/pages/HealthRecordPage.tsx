...
export function HealthRecordPage() {
  ...
  async function loadReportBatch(cursor: string | null, direction: "next" | "previous") {
    if (!isUuidV7(caseId)) return;
    setReportLoading(true);
    try {
      const reports = await listInstitutionDetectionReports(caseId, { cursor: cursor ?? undefined, limit: 20 });
      setBundle((current) => (current ? { ...current, reports } : current));
      if (direction === "next") {
        setCursorHistory((current) => [...current.slice(0, cursorIndex + 1), cursor]);
  ...
  return (
    <main className="mx-auto w-full max-w-[1280px] space-y-5">
      <Link className="inline-flex items-center text-sm font-medium text-teal-700" to="/institution/member-enrollments">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold tracking-wide text-teal-700">会员服务 / 评估输入准备</p>
          <h1 className="mt-1 text-2xl font-semibold text-slate-950">健康档案与评估准备</h1>
      ...
      {feedback && !bundle ? (
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-panel">
          ...
          <p className="mt-2 text-sm text-slate-600">请确认网络与登录状态后重新加载；系统不会把失败状态当作已完成。</p>
        ...
        <>
          ...
          <section className="grid gap-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(320px,0.7fr)]">
            <div className="space-y-5">
              <IndicatorPanel indicators={bundle.indicators} states={bundle.record.indicator_states} />
              <ReportPanel
                cursorIndex={cursorIndex}
                loading={reportLoading}
                onNext={() =>
                  bundle.reports.next_cursor ? void loadReportBatch(bundle.reports.next_cursor, "next") : undefined
                }
                onPrevious={() => void loadReportBatch(cursorHistory[cursorIndex - 1] ?? null, "previous")}
                page={bundle.reports}
              />
            </div>
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel">
      ...
      {!complete ? (
        <ul className="mt-3 space-y-2 text-sm text-slate-700">
          {record.missing_section_codes.map((code) => (
            <li className="rounded-lg border border-amber-100 bg-amber-50/60 px-3 py-2" key={code}>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 text-sm">
        <SummaryField label="服务指标" value={`${record.indicator_codes.length} 项`} />
        <SummaryField label="报告记录" value={`${record.report_metadata_count} 份`} />
        <div className="col-span-2">
          <SummaryField label="最近更新" value={formatTime(record.updated_at)} />
        </div>
      </dl>
    </article>
  );
...
function StatusRail({ bundle }: { bundle: HealthRecordBundle }) {
  ...
  const steps = [
    { label: "档案摘要", value: complete ? "档案已完整" : "档案待补充", done: complete },
    { label: "服务指标", value: `${bundle.indicators.length} 项最新数据`, done: bundle.indicators.length > 0 },
    { label: "数据汇总", value: current ? "已同步" : "等待同步", done: current },
    { label: "评估条件", value: readinessTitle(bundle.readiness.status), done: ready },
  ];
  return (
    <section
      aria-label="评估输入链"
    ...
    >
      {steps.map((step, index) => (
        <div className="relative border-slate-100 px-5 py-4 sm:border-r" key={step.label}>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            ...
            >
              {step.done ? <CheckCircle2 aria-hidden="true" size={15} /> : <Clock3 aria-hidden="true" size={15} />}
            </span>
            {index + 1}. {step.label}
          </div>
...
function IndicatorPanel({ indicators, states }: { indicators: HealthIndicatorFact[]; states: Record<string, string> }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-white shadow-panel">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">由服务端按当前服务范围筛选；页面不自行计算有效性或 BMI。</p>
        </div>
          ...
          {indicators.map((item) => (
            <div className="bg-white p-5" key={item.fact_ref}>
              <div className="flex items-start justify-between gap-3">
                <p className="text-sm font-medium text-slate-600">{indicatorLabel(item.indicator_code)}</p>
                <StateBadge state={states[item.indicator_code] ?? item.verification_state} />
              </div>
              <p className="mt-3 text-2xl font-semibold tabular-nums text-slate-950">
                {String(item.value)} <span className="text-sm font-medium text-slate-500">{unitLabel(item.unit)}</span>
              </p>
              <p className="mt-2 text-xs text-slate-400">
                {item.indicator_code === "bmi" ? "服务端已计算" : `测量于 ${formatTime(item.measured_at)}`}
              </p>
            </div>
          ))}
        </div>
      ) : (
        <p className="px-5 py-10 text-center text-sm text-slate-500">当前服务范围内暂无可展示的最新指标。</p>
      )}
    </article>
  );
}
    ...
    <article className="rounded-xl border border-slate-200 bg-white shadow-panel">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">仅展示报告类型、时间和处理状态，不提供报告原件。</p>
        </div>
      ...
      {page.items.length ? (
        <div className="divide-y divide-slate-100">
          {page.items.map((report) => (
            ...
            >
              <div>
                <p className="font-medium text-slate-950">{reportTypeLabel(report.report_type)}</p>
                <p className="mt-1 text-xs text-slate-500">检测于 {formatTime(report.measured_at)}</p>
              </div>
              <span className="text-sm text-slate-600">{report.attachment_count} 个附件</span>
...
function ReadinessPanel({ readiness }: { readiness: AssessmentReadiness }) {
  ...
  const details = [
    ...readiness.missing_indicator_codes.map((code) => `缺少：${indicatorLabel(code)}`),
    ...readiness.expired_indicator_codes.map((code) => `已过期：${indicatorLabel(code)}`),
    ...readiness.disputed_indicator_codes.map((code) => `待确认：${indicatorLabel(code)}`),
  ];
  if (readiness.status === "DATA_SYNC_PENDING") details.push("系统正在汇总最新数据，请稍后刷新。 ");
  return (
    <article
      className={`rounded-xl border p-5 shadow-panel ${ready ? "border-teal-200 bg-teal-50/70" : "border-amber-200 bg-amber-50/70"}`}
    >
      <div className="flex items-start gap-3">
        <span className={`rounded-lg p-2 ${ready ? "bg-teal-100 text-teal-700" : "bg-amber-100 text-amber-700"}`}>
          {ready ? <ClipboardCheck aria-hidden="true" size={20} /> : <CircleAlert aria-hidden="true" size={20} />}
        </span>
        <div>
          <p className="text-xs font-semibold tracking-wide text-slate-500">评估准备状态</p>
          <h2 className="mt-1 text-lg font-semibold text-slate-950">{readinessTitle(readiness.status)}</h2>
        </div>
      </div>
      {ready ? (
        ...
        <p className="mt-4 text-sm text-slate-700">当前状态尚未具备可执行的评估输入，请稍后刷新。</p>
      )}
      <dl className="mt-4 grid gap-3 border-t border-slate-200/70 pt-4 text-sm sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
        <SummaryField label="数据截至" value={formatOptionalTime(readiness.data_as_of)} />
        <SummaryField label="状态生成" value={formatOptionalTime(readiness.generated_at)} />
      </dl>
    </article>
  );
}
...
function missingSectionLabel(code: string) {
  return ({ HEALTH_PROFILE: "基础健康档案待补充" } as Record<string, string>)[code] ?? "档案模块待补充";
}

function reportStatusLabel(status: string) {
  return (
    ({ READY: "已归档", ACTIVE: "有效", CLEAN: "安全可用", SUPERSEDED: "已有更新版本" } as Record<string, string>)[
      status
    ] ?? "已记录"
  );
}
