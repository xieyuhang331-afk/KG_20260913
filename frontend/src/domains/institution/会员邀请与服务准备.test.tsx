...
describe("机构客户服务邀约与服务接入", () => {
  ...
  it.each([
    ...
    await screen.findByText("110***********1234");
    expect(screen.queryByRole("button", { name: "确认并提交核验" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "创建分配" })).not.toBeInTheDocument();
    if (identityStatus === "UNRECOGNIZED_STATE") {
      expect(screen.getByRole("alert")).toHaveTextContent("状态信息暂不一致");
    }
  });
  ...
  it("必要同意、已接受分配和业务名称形成完整服务接入流程", async () => {
    ...
    expect(screen.getByText("服务同意已确认").closest("div")).toHaveTextContent("已完成");
    expect(screen.getByText("主健管师已接受").closest("div")).toHaveTextContent("已完成");
  ...
  it("只有待接受分配允许取消且不能提前标记完成", async () => {
    ...
    expect(screen.getByText("等待健管师接受").closest("div")).toHaveTextContent("当前状态");
  });

  it.each(["ACCEPTED", "DECLINED", "CANCELLED"])("分配状态 %s 不显示取消操作", async (status) => {
    mockSequence(
      success(
        enrollmentDetail({
  ...
  it("服务案例缺少必要同意时显示受控错误且停用操作", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("状态信息暂不一致");
    expect(screen.queryByText("服务准备中")).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: /查看客户健康档案/ })).not.toBeInTheDocument();
...
function acceptedConsents() {
  return ["USER_AGREEMENT", "PRIVACY_POLICY", "HEALTH_DATA_PROCESSING", "INSTITUTION_SERVICE", "NON_MEDICAL_RISK"].map(
    (documentType, index) => ({
