...
describe("机构健康档案与评估准备", () => {
  ...
  it("冻结四个只读API路径且不发送请求体", async () => {
    ...
    for (const [, options] of fetchMock.mock.calls as Array<[string, RequestInit | undefined]>) {
  ...
  it("从已创建服务案例的入组详情进入健康档案", async () => {
    mockByPath({
      [`/api/v1/institution/member-enrollments/${enrollmentId}`]: enrollmentDetail(),
    ...
    render(
      <MemoryRouter initialEntries={[`/institution/member-enrollments/${enrollmentId}`]}>
        <Routes>
    ...
    expect(await screen.findByRole("link", { name: "查看健康档案与评估准备" })).toHaveAttribute(
      "href",
  ...
  it("以正向状态展示最小档案摘要、服务端指标和报告元数据", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "健康档案与评估准备" })).toBeInTheDocument();
    ...
    expect(screen.queryByRole("link", { name: /原件|下载|查看附件/ })).not.toBeInTheDocument();
    expect(screen.queryByText(/病史详情|过敏详情|用药详情|症状详情/)).not.toBeInTheDocument();
    expect(screen.queryByText(/^(评估结果|风险结论|健康方案)$/)).not.toBeInTheDocument();
  });
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.endsWith("/health-record")) {
        ...
        return Promise.resolve(
          recordAttempts === 1 ? failure(503, "DEPENDENCY_UNAVAILABLE", "database secret") : success(healthRecord()),
        );
      }
      if (url.includes("/detection-reports")) return Promise.resolve(success(reportPage()));
      if (url.endsWith("/health-indicators/latest")) return Promise.resolve(success(latestIndicators()));
      if (url.endsWith("/assessment-readiness")) return Promise.resolve(success(readiness("ASSESSMENT_READY")));
  ...
  it("报告列表使用opaque cursor翻批且不虚构总页数", async () => {
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith("/health-record")) return Promise.resolve(success(healthRecord()));
      if (url.includes("/detection-reports"))
        return Promise.resolve(success(url.includes("cursor=opaque-next") ? next : first));
      if (url.endsWith("/health-indicators/latest")) return Promise.resolve(success(latestIndicators()));
      if (url.endsWith("/assessment-readiness")) return Promise.resolve(success(readiness("ASSESSMENT_READY")));
      throw new Error(`UNEXPECTED_TEST_PATH:${url}`);
    });
    ...
    await waitFor(() =>
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes("cursor=opaque-next"))).toBe(true),
    );
...
function renderHealthPage() {
  return render(
    <MemoryRouter initialEntries={[`/institution/service-cases/${caseId}/health-record`]}>
      <Routes>
        <Route element={<HealthRecordPage />} path="/institution/service-cases/:caseId/health-record" />
      </Routes>
    </MemoryRouter>,
  );
}

function mockHealthPage(overrides: { record?: object; readiness?: object; reports?: object } = {}) {
  const fetchMock = vi.fn((input: RequestInfo | URL) => {
    const url = String(input);
    if (url.endsWith("/health-record")) return Promise.resolve(success(overrides.record ?? healthRecord()));
    if (url.includes("/detection-reports")) return Promise.resolve(success(overrides.reports ?? reportPage()));
    if (url.endsWith("/health-indicators/latest")) return Promise.resolve(success(latestIndicators()));
    if (url.endsWith("/assessment-readiness"))
      return Promise.resolve(success(overrides.readiness ?? readiness("ASSESSMENT_READY")));
    throw new Error(`UNEXPECTED_TEST_PATH:${url}`);
  });
  vi.stubGlobal("fetch", fetchMock);
  return fetchMock;
...
function mockByPath(values: Record<string, object>) {
  const fetchMock = vi.fn((input: RequestInfo | URL) => {
    const path =
      new URL(String(input), "http://local.test").pathname + new URL(String(input), "http://local.test").search;
...
function fact(code: string, value: string, unit: string, state = "VERIFIED", index = 1) {
  return {
...
function readiness(status: "DATA_INSUFFICIENT" | "DATA_SYNC_PENDING" | "DISPUTED" | "ASSESSMENT_READY") {
  return {
    ...
    projection_status: status === "DATA_SYNC_PENDING" ? "SYNC_PENDING" : "CURRENT",
    data_as_of: "2026-08-23T09:00:00+08:00",
    generated_at: "2026-08-23T09:20:00+08:00",
    assembly_id: status === "ASSESSMENT_READY" ? "0198c4a1-1111-7abc-8000-000000000402" : null,
  };
}
