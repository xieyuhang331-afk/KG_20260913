...
describe("健管师审核", () => {
	beforeEach(() => {
		...
		vi.mocked(platformApi.listTherapistRenewalReviews).mockResolvedValue({
			items: [],
		...
		vi.mocked(platformApi.exitTherapist).mockResolvedValue({
			therapist_id: therapistId,
			status: "EXITED",
			version: 3,
		});
	...
	it("平台导航只向super_admin暴露审核与状态页面", () => {
		const items = platformNavigation.filter((item) =>
			item.path.includes("therapist"),
		);
	...
	it("队列支持初审续期和opaque cursor且详情固定操作栏", async () => {
		...
		await userEvent.click(
			await screen.findByRole("button", { name: "下一批审核" }),
		);
		...
		await userEvent.click(
			await screen.findByRole("button", { name: "查看审核详情" }),
		);
		expect(
			await screen.findByText("当前资料版本：第 2 版"),
		).toBeInTheDocument();
		expect(screen.queryByText(/Revision|QUEUED/)).not.toBeInTheDocument();
		expect(screen.getByRole("region", { name: "审核操作" })).toHaveClass(
			"sticky",
		);
	});
	...
	it("领取审核使用START_REVIEW且操作中禁用决定按钮", async () => {
		...
		await userEvent.click(
			await screen.findByRole("button", { name: "领取审核" }),
		);
		...
		expect(
			vi.mocked(platformApi.decideTherapistReview).mock.calls[0]?.[1],
		).toEqual({
	...
	it("补正重提后只提交当前Revision资格且历史资格只读展示", async () => {
		...
		vi.mocked(platformApi.listTherapistReviews).mockResolvedValueOnce({
			items: [underReview],
		});
		vi.mocked(platformApi.getTherapistReview).mockResolvedValueOnce(
			detailFixture({ review_item: underReview }),
		);
		render(<TherapistReviewPage />);
		await userEvent.click(
			await screen.findByRole("button", { name: "查看审核详情" }),
		...
		const payload = vi.mocked(platformApi.decideTherapistReview).mock
			.calls[0]?.[1];
		expect(payload?.qualification_outcomes).toEqual({
			[currentQualificationId]: "APPROVED",
		});
		expect(payload?.qualification_outcomes).not.toHaveProperty(revisionId);
		expect(payload?.qualification_outcomes).not.toHaveProperty(
			historicalQualificationId,
		);
	...
	it("续期审核调用review_item路由且只提交绑定资格", async () => {
		...
		vi.mocked(platformApi.listTherapistRenewalReviews).mockResolvedValueOnce({
			items: [renewalItem],
		});
		vi.mocked(platformApi.getTherapistReview).mockResolvedValueOnce(
			detailFixture({ review_item: renewalItem }),
		);
		render(<TherapistReviewPage />);
		await userEvent.click(screen.getByRole("button", { name: "续期审核" }));
		await userEvent.click(
		...
		await userEvent.click(screen.getByRole("button", { name: "确认批准" }));
		await waitFor(() =>
			expect(platformApi.decideTherapistRenewalReview).toHaveBeenCalledTimes(1),
		);
		expect(
			vi.mocked(platformApi.decideTherapistRenewalReview).mock.calls[0]?.[0],
		).toBe(reviewItemId);
		expect(
			vi.mocked(platformApi.decideTherapistRenewalReview).mock.calls[0]?.[1]
				.qualification_outcomes,
		).toEqual({
			[currentQualificationId]: "APPROVED",
		});
	...
	it.each([
		...
		vi.mocked(platformApi.getTherapistReview).mockResolvedValueOnce(
			detailFixture({
				current_qualification_ids: currentIds,
				review_item: underReview,
			}),
		);
		render(<TherapistReviewPage />);
		await userEvent.click(
	...
	it("409后只刷新且不自动重放审核决定", async () => {
		...
		await userEvent.click(screen.getByRole("button", { name: "确认批准" }));
		await waitFor(() =>
			expect(platformApi.listTherapistReviews).toHaveBeenCalledTimes(2),
		);
		expect(platformApi.decideTherapistReview).toHaveBeenCalledTimes(1);
	...
	it("暂停恢复退出来自真实详情并均要求二次确认", async () => {
		...
		await userEvent.click(
			await screen.findByRole("button", { name: "查看状态详情" }),
		);
		await userEvent.click(
			await screen.findByRole("button", { name: "暂停服务" }),
		);
