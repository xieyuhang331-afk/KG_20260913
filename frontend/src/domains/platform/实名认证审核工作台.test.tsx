...
describe("Platform Member Identity Review Workbench", () => {
	...
	it("使用 UUIDv7 与 cursor 请求新队列", async () => {
		...
		const value = await listMemberIdentityReviews({
	...
	it("队列只渲染脱敏证件和 UUID 审核标识", async () => {
		mockSequence(
			success(page([{ ...summary(), unexpected_full_value: "forbidden" }])),
		);
		renderList();
	...
	it("503 不显示原始 detail 并允许重新加载", async () => {
		const fetchMock = mockSequence(
			failure(503, "DEPENDENCY_UNAVAILABLE", "internal detail"),
			success(page([])),
		);
		renderList();
		expect(
	...
	it("未重认证前完整身份始终隐藏", async () => {
		...
		expect(
			screen.queryByText("OFFLINE_IDENTITY_CHECKED"),
		).not.toBeInTheDocument();
		expect(screen.queryByText(/Revision/)).not.toBeInTheDocument();
		expect(screen.queryByText(reviewId)).not.toBeInTheDocument();
		expect(screen.queryByText(/0198b963…000022/)).not.toBeInTheDocument();
	});
	...
	it("403 决定失败会立即清除已显示的完整身份且不显示原始错误", async () => {
		...
		mockSequence(
			success(detail()),
			success(pii()),
			failure(403, "REVIEWER_CURRENTNESS_FORBIDDEN", "private detail"),
		);
		renderDetail();
		await screen.findByText("110***********1234");
		await userEvent.type(screen.getByLabelText("当前审核员密码"), "temporary");
		...
		expect(
			await screen.findByText("当前账号无权执行此操作"),
		).toBeInTheDocument();
		expect(screen.queryByText(syntheticFullId)).not.toBeInTheDocument();
	...
	it("409 清除敏感状态、刷新一次且不重放决定", async () => {
		...
		expect(screen.queryByText(syntheticFullId)).not.toBeInTheDocument();
		expect(
			fetchMock.mock.calls.filter(([url]) => String(url).endsWith("/decision")),
		).toHaveLength(1);
	});
...
function renderDetail() {
	return render(
		<MemoryRouter initialEntries={[`/platform/identity-reviews/${reviewId}`]}>
			<Routes>
				<Route
					path="/platform/identity-reviews/:reviewId"
					element={<IdentityReviewDetailPage />}
				/>
			</Routes>
		</MemoryRouter>,
	);
...
function mockSequence(...values: Array<Response | Promise<Response>>) {
	...
	for (const value of values)
		fetchMock.mockImplementationOnce(() => Promise.resolve(value));
