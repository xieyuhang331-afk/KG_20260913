...
import type { TherapistReviewDecisionPayload, TherapistReviewDetail, TherapistReviewItem } from "../types";
...
function message(error: unknown, fallback: string) {
  ...
  if (error.status === 409) return "审核状态已变化，已刷新最新数据，请重新确认。";
...
function currentSet(detail: TherapistReviewDetail) {
  ...
  const groups = ids.map((id) => detail.qualifications.filter((q) => q.qualification_version_id === id));
...
export function TherapistReviewPage() {
  ...
  async function decide() {
    ...
    const payload: TherapistReviewDecisionPayload = {
      ...
      reason_code: decision === "APPROVED" || decision === "START_REVIEW" ? null : "QUALIFICATION_REVIEW",
      profile_fields: decision === "NEEDS_CORRECTION" ? ["practice_summary"] : [],
      qualification_targets: [],
      qualification_outcomes:
        decision === "APPROVED" || decision === "REJECTED"
          ? Object.fromEntries(
              current.map((q) => [q.qualification_version_id, decision === "APPROVED" ? "APPROVED" : "REJECTED"]),
            )
          : {},
    };
    setBusy(true);
    ...
    try {
      if (detail.review_item.review_kind === "RENEWAL")
        await decideTherapistRenewalReview(detail.review_item.review_item_id, payload, crypto.randomUUID());
      else await decideTherapistReview(detail.profile.therapist_id, payload, crypto.randomUUID());
      setPending(null);
      setNotice("审核决定已记录");
      await load(mode);
    } catch (reason) {
  ...
  const history =
    detail?.qualifications.filter((q) => !new Set(detail.current_qualification_ids).has(q.qualification_version_id)) ??
    [];
  return (
    <main className="space-y-5 pb-10">
      <header>
        <p className="text-sm font-medium text-teal-700">平台治理 / 健管师资质</p>
        <h1 className="mt-1 text-2xl font-semibold">健管师资质审核</h1>
        <p className="mt-2 text-sm text-slate-600">审核决定始终基于当前资料版本和服务端状态。</p>
      </header>
      <Feedback message={error || notice} tone={error ? "error" : "success"} />
      <div className="flex gap-2">
        <button
          className={mode === "INITIAL" ? primaryButtonClassName : secondaryButtonClassName}
        ...
        <button
          className={mode === "RENEWAL" ? primaryButtonClassName : secondaryButtonClassName}
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-slate-500">
                      {reviewItemStatusLabel(item.status)} · 第 {item.version} 版
                    </p>
                  </div>
                  <button
                    aria-label="查看审核详情"
        ...
        {detail ? (
          <section className="min-w-0 rounded-xl border bg-white p-5 shadow-panel">
            <p className="text-sm text-slate-500">当前资料版本：第 {detail.profile.current_revision_no} 版</p>
              ...
              <>
                ...
                {current.map((q) => (
                  <p className="mt-2 text-sm" key={q.qualification_version_id}>
                    代谢健康执业资格第 {q.version_no} 版 · {q.masked_certificate_no} · 材料 {q.attachment_count} 份
                  </p>
                ))}
                <h3 className="mt-5 font-semibold">历史版本</h3>
                {history.map((q) => (
                  <p className="mt-2 text-sm text-slate-500" key={q.qualification_version_id}>
                    第 {q.version_no} 版 · {qualificationStatusLabel(q.derived_review_status)}
                  </p>
                ))}
