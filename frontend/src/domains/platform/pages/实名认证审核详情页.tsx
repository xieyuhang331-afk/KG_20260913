...
export function IdentityReviewDetailPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			if (!controller.signal.aborted)
				setError(getSafeApiError(reasonValue).message);
		} finally {
	...
	async function reveal(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			if (!controller.signal.aborted && piiController.current === controller)
				setPii(value);
		} catch (reasonValue) {
			if (!controller.signal.aborted) {
				const safe = getSafeApiError(reasonValue);
	...
	async function submitDecision(event: React.FormEvent<HTMLFormElement>) {
		event.preventDefault();
		if (!detail || !window.confirm("确认提交该审核决定？提交后不会自动重放。"))
			return;
		...
		const payload = {
			...
			represented_elder_eligible:
				decision === "APPROVED" && detail.mode === "PROXY_ELDER" ? true : null,
					...
					<div className="space-y-5">
						<section className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel">
							...
							<dl className="mt-5 grid gap-4 sm:grid-cols-2">
								...
								<Field
									label="入组模式"
									value={detail.mode === "SELF" ? "本人" : "代办老人"}
								/>
								<Field label="当前实名材料" value="当前提交版本" />
								<Field
									label="材料版本"
									value={`第 ${detail.current_revision_no} 版`}
								/>
								<Field
									label="机构核验声明"
									value={attestationLabel(detail.institution_attestation)}
								/>
								<Field
									label="代理见证状态"
									value={detail.proxy_witness_status ?? "不适用"}
								/>
							</dl>
							<button
								...
								<>
									<label className="block text-sm font-medium">
										...
										<select
											...
											value={reason}
										>
											<option value="IDENTITY_INFORMATION_INCONSISTENT">
												身份信息不一致
											</option>
											<option value="IDENTITY_DOCUMENT_INVALID">
												证件无效
											</option>
											<option value="PROXY_EVIDENCE_INCOMPLETE">
												代理证明不完整
											</option>
										</select>
									</label>
...
function Field({
	...
	return (
		<div>
			<dt className={`text-xs ${dark ? "text-slate-400" : "text-slate-400"}`}>
				{label}
			</dt>
			<dd
				className={`mt-1 break-all font-medium ${dark ? "text-white" : "text-slate-800"}`}
			>
				{value}
			</dd>
		</div>
...
function attestationLabel(value: string | null) {
	...
	if (value === "PRINCIPAL_PRESENT_AND_AUTHORIZED_PROXY")
		return "本人在场并已授权代办";
	return "未提供";
}
