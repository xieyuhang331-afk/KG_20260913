...
export function InstitutionInvitationPage() {
	...
	const load = useCallback(async () => {
		...
		setLoadError("");
		try {
			const [invitations, tree] = await Promise.all([
				listInstitutionInvitations(),
				listOrganizationTree(false),
			]);
			setRows(invitations);
	...
	async function submit(event: React.FormEvent<HTMLFormElement>) {
		...
		const payload: InstitutionInvitationPayload = {
			...
			) as InstitutionInvitationPayload["institution_type"],
		...
		try {
			const value = await createInstitutionInvitation(
				payload,
				crypto.randomUUID(),
			);
			...
			setTone("success");
			setMessage(
				"邀请已创建。一次性短码只显示在当前页面，请通过批准的线下方式交付。 ",
			);
			await load();
		} catch (error) {
			setTone("error");
			setMessage(
				onboardingErrorMessage(error, "邀请创建失败，请检查填写内容后重试。"),
			);
		} finally {
			setBusy(false);
		}
	...
	async function confirmMutation() {
		...
		try {
			...
			const value =
				operation === "resend"
					? await resendInstitutionInvitation(
							row.invitation_id,
							row.version,
							crypto.randomUUID(),
						)
					: await revokeInstitutionInvitation(
							row.invitation_id,
							row.version,
							crypto.randomUUID(),
						);
		...
		<main className="space-y-5">
			...
			<div className="grid items-start gap-5 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
				<section className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel xl:sticky xl:top-24">
					...
					<form className="mt-5 grid gap-4" onSubmit={submit} ref={formRef}>
						...
						<Field
							controlId="applicant-phone"
							hint="仅用于冻结本次邀请，不会展示在邀请列表。"
							label="受邀手机号"
						>
							<input
								autoComplete="off"
						...
						<button
							...
							disabled={busy || counties.length === 0}
							type="submit"
						>
							{busy ? "正在创建…" : "创建邀请"}
						</button>
				...
				<section className="min-w-0 space-y-4">
						...
						>
							...
							<p className="mt-2 text-xs text-amber-800">
								...
								{issuedCode.expiresAt
									? `，到期时间：${formatTime(issuedCode.expiresAt)}`
									: ""}
								。
							</p>
							<button
					...
					{loading ? (
						...
						<EmptyPanel
							description="还没有机构邀请。填写左侧信息创建第一条限时邀请。"
							title="暂无邀请记录"
						/>
					) : (
										...
										{row.status === "ISSUED" ? (
											<div className="flex gap-2">
												<button
													...
													disabled={busy}
													onClick={() =>
														setPendingAction({ operation: "resend", row })
													}
													type="button"
												>
													<RotateCw
												...
												<button
													...
													disabled={busy}
													onClick={() =>
														setPendingAction({ operation: "revoke", row })
													}
													type="button"
												>
													<ShieldX
			...
			{pendingAction ? (
				<ConfirmDialog
					busy={busy}
					confirmLabel={
						pendingAction.operation === "resend" ? "确认重发" : "确认撤销"
					}
					description={
...
function flattenActiveCounties(
	nodes: OrganizationTreeNode[],
): OrganizationTreeNode[] {
	return nodes.flatMap((node) => [
		...(node.org_type === "county" &&
		node.status === "active" &&
		node.compatibility_mode === "canonical"
			? [node]
			: []),
...
function invitationStatusLabel(status: string) {
	return (
		{ ISSUED: "待激活", ACTIVATED: "已激活", REVOKED: "已撤销" }[status] ??
		status
	);
}
