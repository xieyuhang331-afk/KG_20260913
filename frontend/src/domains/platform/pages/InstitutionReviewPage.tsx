import { CheckCircle2, FileLock2, RefreshCw, Search, ShieldAlert } from "lucide-react";
...
const draftFields = correctionOptions.filter(([key]) => !key.endsWith("license"));
...
export function InstitutionReviewPage() {
  ...
  async function viewMaterial(fileId: string) {
    ...
    try {
      ...
      setTone("error");
      setMessage(onboardingErrorMessage(error, "材料读取失败，请重新验证密码。"));
    } finally {
      setBusy(false);
    }
  }
  ...
  function prepareDecision(decision: Exclude<PendingDecision, null>) {
    if (decision === "NEEDS_CORRECTION" && selectedCorrectionFields.length === 0) {
      setTone("error");
    ...
    try {
      await decideInstitutionReview(
        applicationId,
        {
          ...
          correction_fields: decision === "NEEDS_CORRECTION" ? selectedCorrectionFields : [],
          reason_code:
            decision === "APPROVED"
              ? null
      ...
      setPendingDecision(null);
      setTone("error");
      setMessage(onboardingErrorMessage(error, "审核决定提交失败，请确认当前状态。"));
  ...
  return (
    <main className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold tracking-[0.16em] text-teal-700">APPLICATION REVIEW</p>
          <h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">机构入驻审核</h1>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-panel">
          <div className="border-b border-slate-100 px-5 py-4">
            ...
            <p className="mt-1 text-xs text-slate-500">选择申请后在右侧完成审核</p>
          </div>
          {loadingList ? (
            <div className="p-4">
            ...
            <div className="p-4">
              <EmptyPanel description="当前没有待审核申请。稍后刷新可获取新提交。" title="队列已处理完" />
            </div>
          ) : (
                  ...
                  >
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block text-xs text-slate-500">
                        {row.submitted_at ? formatTime(row.submitted_at) : "等待提交时间"}
                      </span>
                    </span>
                    <span
        ...
        <section className="min-w-0">
          {loadingDetail ? (
            ...
            <EmptyPanel
              action={<Search aria-hidden="true" className="mx-auto text-slate-400" size={24} />}
            ...
            <div className="space-y-5">
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold text-slate-400">申请编号 {shortId(detail.application_id)}</p>
                    <h2 className="mt-2 text-xl font-semibold text-slate-950">申请详情</h2>
                  </div>
                  <span
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h2 className="font-semibold text-slate-950">受控材料查看</h2>
                ...
                <label className="mt-4 block max-w-md text-sm font-medium text-slate-700">
                  当前账户密码
                  <input autoComplete="current-password" className={fieldClassName} ref={passwordRef} type="password" />
                </label>
                <div className="mt-4 space-y-3">
                  {materials.length ? (
                    materials.map((material) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">扫描状态：{scanStatusLabel(material.status)}</p>
                        </div>
                        <button
                          aria-label="受控查看材料"
                          className={secondaryButtonClassName}
                    ...
                    <p className="text-sm text-slate-500">当前申请没有可查看材料。</p>
                  )}
                </div>
              </section>
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-panel">
                ...
                <fieldset className="mt-4 rounded-lg border border-slate-200 p-4">
                  <legend className="px-1 text-sm font-semibold text-slate-700">需要补正的字段</legend>
      ...
      {pendingDecision ? (
        <ConfirmDialog
          ...
          description={decisionDescription(pendingDecision, selectedCorrectionFields.length)}
...
function licenseLabel(type: string) {
  return type === "MEDICAL_INSTITUTION_LICENSE" ? "医疗机构执业许可证" : "营业执照";
}
...
function decisionButtonLabel(decision: Exclude<PendingDecision, null>) {
  return decision === "APPROVED" ? "确认批准" : decision === "NEEDS_CORRECTION" ? "确认补正" : "确认拒绝";
}
function decisionDescription(decision: Exclude<PendingDecision, null>, count: number) {
  return decision === "APPROVED"
    ? "批准后将创建并激活 Tenant，但 SERVICE_READY 仍为 false。"
    : decision === "NEEDS_CORRECTION"
