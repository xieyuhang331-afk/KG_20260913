...
def test_Module_D五个READY视图与最小权限闭环(pg_database):
    assert pg_database.fetch_value("SELECT version_num='20260914_0047' FROM alembic_version")
...
def test_Module_D真实运行身份与membership预检失败保持零DDL(pg_database, monkeypatch):
    ...
    try:
        ...
        command.upgrade(config, "head")
        assert pg_database.fetch_value(
            "SELECT version_num='20260914_0047' FROM alembic_version"
        )
        asyncio.run(admin_execute(f'GRANT "{writer}" TO "{organization_reader}"'))
        try:
            with pytest.raises(RuntimeError) as error:
        ...
        if pg_database.fetch_value("SELECT version_num FROM alembic_version") != "20260914_0047":
            command.upgrade(config, "head")
