...
def test_Module_D五个READY视图与最小权限闭环(pg_database):
    assert pg_database.fetch_value("SELECT version_num='20260904_0037' FROM alembic_version")
...
def test_Module_D真实运行身份与membership预检失败保持零DDL(pg_database, monkeypatch):
    ...
    try:
        ...
        if pg_database.fetch_value("SELECT version_num FROM alembic_version") != "20260904_0037":
            command.upgrade(config, "head")
