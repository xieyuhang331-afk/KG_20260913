...
async def _read_privileges(database_url: str) -> dict[str, bool]:
    ...
    try:
        row = await connection.fetchrow(
            ...
            "SELECT "
            "MAX(c.oid) FILTER ("
            "WHERE n.nspname = 'identity' AND c.relname = 'member'"
            ") AS member_oid, "
            "MAX(c.oid) FILTER ("
            "WHERE n.nspname = 'public' AND c.relname = 'alembic_version'"
            ") AS alembic_version_oid "
            "FROM pg_class AS c "
            ...
            "AS schema_create, "
            "has_table_privilege(current_user, member_oid, 'SELECT') "
            "AS member_select, "
            "has_table_privilege(current_user, member_oid, 'INSERT') "
            "AS member_insert, "
            "has_table_privilege(current_user, member_oid, 'UPDATE') "
            "AS member_update, "
            "has_table_privilege(current_user, member_oid, 'DELETE') "
            "AS member_delete, "
            "has_table_privilege(current_user, member_oid, 'TRUNCATE') "
            "AS member_truncate, "
            "has_table_privilege(current_user, member_oid, 'REFERENCES') "
            "AS member_references, "
            "has_table_privilege(current_user, member_oid, 'TRIGGER') "
            "AS member_trigger, "
            "has_table_privilege(current_user, alembic_version_oid, "
