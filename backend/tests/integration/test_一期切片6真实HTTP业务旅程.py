...
from app.core.database import get_slice6_template_writer_session
...
def test_API01_API10_真实ASGI完成机构发起平台审核家庭确认旅程(
    ...
    decision_headers = {
        **_headers(int(seeded["expert_actor_id"]), "expert"),
        "Idempotency-Key": "slice6-http-review-decision-0001",
    }
    ...
    assert [row["plan_id"] for row in family_list.json()["items"]] == [plan_id]
